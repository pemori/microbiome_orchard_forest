# =============================================================================
# Script: 07_hypothesis_3_16S_assembly.R
...
#
# Description:
# This script evaluates Hypothesis 3 for the bacterial community in orchard
...
# bacterial genera differ in niche breadth, abundance structure, and inferred
# assembly processes under contrasting forest-cover contexts.
#
# Specifically, the script:
#   - compares mean bacterial relative abundance between orchard and forest
...
#     pNST analyses;
#   - fits a Sloan neutral model to orchard bacterial communities and
...
#   - results_16S_hyp2/orchard_summary_with_mntd.csv
#
# Output files:
#   - written to the folder "results_16S_hyp3"
# =============================================================================
...
input_phyloseq_asv   <- "results_16S/phyloseq_16S_asv_filtered.rds"
input_phyloseq_genus <- "results_16S/phyloseq_16S_genus.rds"
input_mapping        <- "results_16S/mapping_16S_ordered.csv"
input_otu_crops      <- "results_16S/otu_crops_16S_genus.csv"
input_otu_forest     <- "results_16S/otu_forest_16S_genus.csv"
input_family_ra      <- "results_16S/otu_table_16S_family_relative_abundance.csv"
input_orchard_hyp2   <- "results_16S_hyp2/orchard_summary_with_mntd.csv"
outdir               <- "results_16S_hyp3"
...
otu_crops_g_bac  <- read.csv(input_otu_crops, row.names = 1, check.names = FALSE)
otu_forest_g_bac <- read.csv(input_otu_forest, row.names = 1, check.names = FALSE)
data_fam_b_RA    <- read.csv(input_family_ra, row.names = 1, check.names = FALSE)
...
phylo_crops_g_bac  <- subset_samples(data_phylo_genus, cat != "F")
phylo_forest_g_bac <- subset_samples(data_phylo_genus, cat == "F")

mapping_orchard <- as(sample_data(phylo_crops_bac), "data.frame")
mapping_orchard$SampleID <- rownames(mapping_orchard)

mapping_forest <- as(sample_data(phylo_forest_bac), "data.frame")
mapping_forest$SampleID <- rownames(mapping_forest)

# =============================================================================
# Hypothesis 3
# Compared microbial assembly process, niche breadth distributions, and
# abundance structure of bacterial genera in orchard soils with contrasting
...
shared_families <- intersect(names(promedio_abundancia_b_forest), names(promedio_abundancia_b_crops))

df_comparacion_family <- data.frame(
  Family = shared_families,
  Mean_Forest = promedio_abundancia_b_forest[shared_families],
  Mean_Orchard = promedio_abundancia_b_crops[shared_families]
)
...
env_pca_df <- as(sample_data(phylo_crops_bac), "data.frame")[,
...
pca_bact <- PCA(env_pca_df_complete, graph = FALSE)

capture.output(pca_bact$eig, file = file.path(outdir, "pca_env_eigenvalues.txt"))
write.csv(pca_bact$var$coord, file.path(outdir, "pca_env_variable_coordinates.csv"))
...
tab <- as(sample_data(phylo_crops_bac), "data.frame")
ID <- rownames(tab)
...
env_bact2 <- data.frame(
  ...
  disp_bact_250 = as.numeric(tab$P_250),
  disp_bact_1000 = as.numeric(tab$P_1000),
  disp_bact_2500 = as.numeric(tab$P_2500),
  check.names = FALSE
)
rownames(env_bact2) <- env_bact2$ID

stopifnot(all(env_bact2$ID %in% rownames(otu_crops_RA)))
otu_crops_RA <- otu_crops_RA[env_bact2$ID, , drop = FALSE]

niche_bact_crops <- cbind(
  data.frame(ID = env_bact2$ID, check.names = FALSE),
  otu_crops_RA,
  env_bact2[, c("pH", "SOM", "N", "Clay+Silt", "disp_bact_2500", "Block_crop"), drop = FALSE]
)

genus_cols <- colnames(otu_crops_RA)

niche_bact_crops[, c(genus_cols, "pH", "SOM", "N", "Clay+Silt", "disp_bact_2500")] <-
  lapply(
    niche_bact_crops[, c(genus_cols, "pH", "SOM", "N", "Clay+Silt", "disp_bact_2500"), drop = FALSE],
    as.numeric
  )

colfreq_idx <- which(colnames(niche_bact_crops) %in% genus_cols)
colvar_idx  <- which(colnames(niche_bact_crops) %in% c("pH", "SOM", "N", "Clay+Silt"))
...
bacti_pal_graph <- data.frame(
  ID = rownames(otu_crops_RA),
  otu_crops_RA,
  check.names = FALSE
) %>%
  left_join(env_bact2, by = "ID")
...
bacti_pal_graph_long <- bacti_pal_graph %>%
  ...
  left_join(
    nb_genus_df %>% select(Genus, pH_nb, SOM_nb, N_nb, claysilt_nb, geometric_mean_nb, NB_Category),
    by = "Genus"
  ) %>%
  mutate(
...
write.csv(
  ...
  file.path(outdir, "bacterial_niche_breadth_long_table.csv"),
  row.names = FALSE
)

bacti_check_sum <- bacti_pal_graph_long %>%
  group_by(ID) %>%
  summarise(total_RA = sum(Relative_Abundance, na.rm = TRUE), .groups = "drop") %>%
  arrange(desc(total_RA))

write.csv(bacti_check_sum, file.path(outdir, "check_sample_relative_abundance_sums.csv"), row.names = FALSE)

# sample counts by forest coverage
sample_count_fc <- bacti_pal_graph_long %>%
...
# genera counts by niche breadth category
...
for (nm in c("Generalist", "Opportunist", "Specialist")) {
  if (!nm %in% colnames(bacti_sum_wide)) bacti_sum_wide[[nm]] <- 0
}

bacti_sum_wide <- bacti_sum_wide %>%
  mutate(total = Generalist + Opportunist + Specialist)

write.csv(bacti_sum_wide, file.path(outdir, "sample_relative_abundance_by_nb_category_wide.csv"), row.names = FALSE)

# =============================================================================
# Summary plots from sample-level sums
# =============================================================================

bacti_plot_summary <- bacti_sum_per_sample %>%
...
write.csv(bacti_plot_summary, file.path(outdir, "summary_mean_se_by_nb_and_forestcover.csv"), row.names = FALSE)
...
Generalist_summary  <- bacti_plot_summary %>% filter(NB_Category == "Generalist")
Opportunist_summary <- bacti_plot_summary %>% filter(NB_Category == "Opportunist")
Specialist_summary  <- bacti_plot_summary %>% filter(NB_Category == "Specialist")
...
ggsave(file.path(outdir, "generalist_bac.png"),  p_generalist,  width = 2.5, height = 4.5, dpi = 300)
ggsave(file.path(outdir, "opportunist_bac.png"), p_opportunist, width = 2.5, height = 4.5, dpi = 300)
ggsave(file.path(outdir, "specialist_bac.png"),  p_specialist,  width = 2.5, height = 4.5, dpi = 300)

# =============================================================================
# Join niche-breadth category abundances to orchard_summary_df
# =============================================================================

RA_by_cat <- bacti_sum_per_sample %>%
  dplyr::select(ID, NB_Category, Sum_Relative_Abundance) %>%
  pivot_wider(
    names_from = NB_Category,
    values_from = Sum_Relative_Abundance,
...
# merged table for downstream use
nb_bac_df <- orchard_summary_df %>%
  left_join(
    orchard_env_dist %>% select(SampleID, env_dist, env_dist_L1),
    by = "SampleID"
  )

write.csv(nb_bac_df, file.path(outdir, "nb_bacterial_model_dataframe.csv"), row.names = FALSE)
...
# clean orchard_env_dist
orchard_env_dist2 <- orchard_env_dist %>%
  dplyr::select(SampleID, env_dist, env_dist_L1)

# clean RA_by_cat (use beta-transformed variables for Beta family)
...
# remove any previous duplicated columns if they exist
...
# keep rownames as SampleID
stopifnot("SampleID" %in% colnames(nb_df))
stopifnot(!anyDuplicated(nb_df$SampleID))
rownames(nb_df) <- nb_df$SampleID

write.csv(nb_df, file.path(outdir, "nb_bacterial_model_dataframe.csv"), row.names = FALSE)
...
marginal_effects(model_Spe4, surface = TRUE)
...
marginal_effects(model_Opp4, surface = TRUE)
...
marginal_effects(model_Gen4, surface = TRUE)
...
save.wd <- normalizePath(file.path(getwd(), outdir, "iCAMP_16S"), winslash = "/", mustWork = FALSE)
...
# now compute phylogenetic distance matrix
...
if (!dir.exists(save.wd)) {
  stop("Could not create directory: ", save.wd)
}
...
orchard_phy <- phylo_crops_bac

tree <- phy_tree(orchard_phy)
if (is.null(tree)) stop("phylo_crops_bac has no phylogenetic tree.")
...
sloan_outdir <- file.path(outdir, "sloan_16S")
...
orchard_asvs <- rownames(otu_orch_all)[rowSums(otu_orch_all) > 0]
forest_asvs  <- rownames(otu_forest_all)[rowSums(otu_forest_all) > 0]
...
sloan_above_bac <- df_neutral %>% filter(Sloan_Class == "Above")
sloan_neutral_bac <- df_neutral %>% filter(Sloan_Class == "Neutral")
sloan_below_bac <- df_neutral %>% filter(Sloan_Class == "Below")

write.csv(sloan_above_bac,   file.path(sloan_outdir, "sloan_above_taxa.csv"),   row.names = FALSE)
write.csv(sloan_neutral_bac, file.path(sloan_outdir, "sloan_neutral_taxa.csv"), row.names = FALSE)
write.csv(sloan_below_bac,   file.path(sloan_outdir, "sloan_below_taxa.csv"),   row.names = FALSE)

ids_sloan_above_shared <- intersect(sloan_above_bac$Taxon, shared_asvs)
ids_sloan_above_unique <- intersect(sloan_above_bac$Taxon, orchard_unique_asvs)

df_sloan_shared <- data.frame(Taxon = ids_sloan_above_shared, stringsAsFactors = FALSE) %>%
  left_join(sloan_above_bac, by = "Taxon") %>%
...
# Denominator is always the full orchard community (phylo_crops_bac)
# -----------------------------------------------------------------------------
...
shared_taxa_ok <- intersect(df_sloan_shared$Taxon, rownames(otu_base_rel))
...
if (length(shared_taxa_ok) > 0) {
  for (nb in unique(shared_nb_lookup$NB_Category)) {
    ...
    ra_nb <- colSums(otu_base_rel[taxa_nb, , drop = FALSE], na.rm = TRUE)
...
unique_taxa_ok <- intersect(df_sloan_unique$Taxon, rownames(otu_base_rel))
...
writeLines(
  capture.output(sessionInfo()),
  file.path(sloan_outdir, "sessionInfo_sloan_16S.txt")
)
