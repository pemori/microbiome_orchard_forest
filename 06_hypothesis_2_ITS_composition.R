# =============================================================================
# Script: 06_hypothesis_2_ITS_composition.R
...
#
# Description:
# This script evaluates Hypothesis 2 for the fungal community:
...
#   - results_ITS_hyp1/orchard_summary_ITS.csv
#   - results_ITS_hyp1/forest_summary_ITS.csv
#
# Output files:
#   - written to the folder "results_ITS_hyp2"
# =============================================================================
...
input_orchard  <- "results_ITS_hyp1/orchard_summary_ITS.csv"
input_forest   <- "results_ITS_hyp1/forest_summary_ITS.csv"
outdir         <- "results_ITS_hyp2"
...
tree_orch <- phy_tree(phylo_crops_fun)
tree_orch_bin <- ape::multi2di(tree_orch, random = TRUE)
phy_tree(phylo_crops_fun) <- tree_orch_bin

tree_forest <- phy_tree(phylo_forest_fun)
tree_forest_bin <- ape::multi2di(tree_forest, random = TRUE)
phy_tree(phylo_forest_fun) <- tree_forest_bin
...
bray_dist_orchard    <- phyloseq::distance(phylo_crops_fun, method = "bray")
unifrac_dist_orchard <- phyloseq::UniFrac(phylo_crops_fun, weighted = TRUE, normalized = TRUE)

bray_dist_forest    <- phyloseq::distance(phylo_forest_fun, method = "bray")
unifrac_dist_forest <- phyloseq::UniFrac(phylo_forest_fun, weighted = TRUE, normalized = TRUE)
...
shared_asvs_tree_orch <- intersect(taxa_names(phylo_crops_fun), phy_tree(phylo_crops_fun)$tip.label)
data_phylo_pruned_orch <- prune_taxa(shared_asvs_tree_orch, phylo_crops_fun)
...
metadata_orch <- data.frame(sample_data(phylo_crops_fun), check.names = FALSE)
...
dataset$tax_table <- subset(dataset$tax_table, Kingdom == "k__Fungi")
...
otu_c <- data.frame(t(otu_table(phylo_crops_fun)))
tax_c <- data.frame(tax_table(phylo_crops_fun))
sam_c <- data.frame(sample_data(phylo_crops_fun), check.names = FALSE)
...
df_diff <- df_diff %>%
  ...
  mutate(Family = ifelse(grepl("f__", Taxa), sub(".*f__", "", Taxa), NA)) %>%
  dplyr::filter(!is.na(Family), Family != "", Family != "NA", Family != "unidentified")
...
t_env$cal_ordination(method = "RDA", taxa_level = "Order")
t_env$trans_ordination(
  show_taxa = 10,
...
res_trans$df_arrows_spe$label <- gsub("^o__", "", res_trans$df_arrows_spe$Order)
...
ggsave(file.path(outdir, "rda_order.png"), p_rda, width = 8, height = 7, dpi = 300)
...
t_env$cal_cor(use_data = "Order", p_adjust_method = "fdr", p_adjust_type = "Env")
...
writeLines(capture.output(sessionInfo()), file.path(outdir, "sessionInfo_hypothesis2_ITS.txt"))